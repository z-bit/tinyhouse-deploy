    ...
    Mutation: {
        ...
        connectStripe: (_root, { input }, { db, req }) => __awaiter(void 0, void 0, void 0, function* () {
            try {
                ...
                if (!wallet)
                    throw new Error('stripe grant failed');
                const update = yield db.users.findOneAndUpdate({ _id: viewer._id }, { $set: { walletId: wallet.stripe_user_id } }, { returnOriginal: false });
                if (!update.value)
        ...
        disconnectStripe: (_root, _args, { db, req }) => __awaiter(void 0, void 0, void 0, function* () {
            try {
                ...
                if (!viewer)
                    throw new Error('viewer cannot be found');
                const update = yield db.users.findOneAndUpdate({ _id: viewer._id }, { $unset: { walletId: '' } }, { returnOriginal: false });
                if (!update.value)
                    throw new Error('viewer could not be updated');
                viewer = update.value || null;
                return {
